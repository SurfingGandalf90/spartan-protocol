  ...
  try {
    ...
    for (const user of users || []) {
      try {
        ...
        const { data: logs } = await supabase.from('session_logs').select('*').eq('user_id', user.id).eq('week', user.current_week || 1)
        ...
        const { error: saveError } = await supabase.from('generated_programs').upsert({
          user_id: user.id, week: nextWeek, program_data: programData, generated_at: new Date().toISOString()
        }, { onConflict: 'user_id,week' })
        if (saveError) throw saveError

        await supabase.from('profiles').update({ current_week: nextWeek }).eq('id', user.id)
